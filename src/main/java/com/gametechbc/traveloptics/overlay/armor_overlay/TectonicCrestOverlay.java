...
public class TectonicCrestOverlay {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (USE_XP_BAR) {
            ...
            poseStack.pushPose();
            guiGraphics.blit(TECTONIC_CREST_TEXTURE, x, y, 29, 89, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * (1.0 - progress));
            if (fillWidth > 0) {
                guiGraphics.blit(TECTONIC_CREST_TEXTURE, x, y, 29, 99, fillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int barWidth = 135;
            ...
            guiGraphics.blit(TECTONIC_CREST_TEXTURE, 0, 0, 53, 19, 135, 22, 241, 115);
            int fillWidth = (int)(135.0 * (1.0 - progress));
            if (fillWidth > 0) {
                guiGraphics.blit(TECTONIC_CREST_TEXTURE, 0, 0, 53, 47, fillWidth, 22, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.getItemBySlot(EquipmentSlot.CHEST);
        if (chestplate.isEmpty() || chestplate.getItem() != TravelopticsItems.TECTONIC_CREST_ARMOR_CHESTPLATE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderFuelBar) {
