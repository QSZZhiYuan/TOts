...
public class PhantomRageOverlay {
    ...
    private static final boolean USE_XP_BAR = ClientConfig.armorReplaceXPBars.get();
    ...
    private static final int BAR_X_OFFSET = ClientConfig.cursedWraithguardXOffset.get();
    private static final int BAR_Y_OFFSET = 95 + ClientConfig.cursedWraithguardYOffset.get();
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (USE_XP_BAR) {
            ...
            poseStack.pushPose();
            guiGraphics.blit(PHANTOM_RAGE_TEXTURE, x, y, 29, 89, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.blit(PHANTOM_RAGE_TEXTURE, x, y, 29, 99, fillWidth, 5, 241, 115);
            }
            if ((cooldownFillWidth = (int)(182.0 * cooldownProgress)) > 0) {
                guiGraphics.blit(PHANTOM_RAGE_TEXTURE, x, y, 29, 78, cooldownFillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int cooldownFillWidth;
            ...
            guiGraphics.blit(PHANTOM_RAGE_TEXTURE, 0, 0, 55, 28, 130, 21, 241, 115);
            int fillWidth = (int)(130.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.blit(PHANTOM_RAGE_TEXTURE, 0, 0, 55, 51, fillWidth, 21, 241, 115);
            }
            if ((cooldownFillWidth = (int)(130.0 * cooldownProgress)) > 0) {
                guiGraphics.blit(PHANTOM_RAGE_TEXTURE, 0, 0, 55, 6, cooldownFillWidth, 21, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.getItemBySlot(EquipmentSlot.CHEST);
        if (chestplate.isEmpty() || chestplate.getItem() != TravelopticsItems.CURSED_WRAITHGUARD_ARMOR_CHESTPLATE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderRageBar) {
