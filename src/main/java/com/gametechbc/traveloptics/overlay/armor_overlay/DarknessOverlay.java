...
public class DarknessOverlay {
    ...
    public static void onRenderOverlay(RenderGuiOverlayEvent.Post event) {
        ...
        if (USE_XP_BAR) {
            ...
            poseStack.pushPose();
            guiGraphics.blit(DARKNESS_TEXTURE, x, y, 29, 89, 182, 5, 241, 115);
            int fillWidth = (int)(182.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.blit(DARKNESS_TEXTURE, x, y, 29, 99, fillWidth, 5, 241, 115);
            }
            poseStack.popPose();
        } else {
            int barWidth = 134;
            ...
            guiGraphics.blit(DARKNESS_TEXTURE, 0, 0, 53, 24, 134, 21, 241, 115);
            int fillWidth = (int)(134.0 * progress);
            if (fillWidth > 0) {
                guiGraphics.blit(DARKNESS_TEXTURE, 0, 0, 53, 55, fillWidth, 21, 241, 115);
            }
            poseStack.popPose();
        }
    }
    ...
    public static void onRenderXpBar(RenderGuiOverlayEvent.Pre event) {
        ...
        ItemStack chestplate = player.getItemBySlot(EquipmentSlot.CHEST);
        if (chestplate.isEmpty() || chestplate.getItem() != TravelopticsItems.FORLORN_HARBINGER_ARMOR_ROBE.get()) {
            return;
        }
        if (USE_XP_BAR) {
            if (!shouldRenderFuelBar) {
