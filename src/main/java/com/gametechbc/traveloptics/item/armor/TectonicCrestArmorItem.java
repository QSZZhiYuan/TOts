...
extends UnbreakableImbueableArmor {
    ...
    private void triggerKnockbackAndDamage(Player player) {
        ...
        List nearbyEntities = level.getNearbyEntities(LivingEntity.class, boundingBox, entity -> {
            ...
            if (entity instanceof TamableAnimal && (tamable = (TamableAnimal)entity).isTame()) {
                return tamable.getOwner() != player;
            }
            return true;
        });
    ...
    private void grantFireResistance(Player player) {
        if (!player.recreateFromPacket(MobEffects.FIRE_RESISTANCE)) {
            player.getStandingEyeHeight(new MobEffectInstance(MobEffects.FIRE_RESISTANCE, 200, 0, false, false));
        }
    }
    ...
    private void setCooldownOnChestplate(Player player) {
        ...
        if (chestplate.setRepairCost() instanceof TectonicCrestArmorItem) {
    ...
    public void resolvePage(ItemStack stack, Level world, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add((Component)Component.translatable((String)"item.traveloptics.tectonic_crest_armor.tooltip").withStyle(ChatFormatting.GREEN));
        tooltip.add((Component)Component.translatable((String)"item.traveloptics.tectonic_crest_armor.tooltip1"));
    ...
    private boolean isWearingFullSet(Player player) {
        return player.shouldRemoveSoulSpeed(ArmorItem.Type.HELMET.getSlot()).setRepairCost() instanceof TectonicCrestArmorItem && player.shouldRemoveSoulSpeed(ArmorItem.Type.CHESTPLATE.getSlot()).setRepairCost() instanceof TectonicCrestArmorItem && player.shouldRemoveSoulSpeed(ArmorItem.Type.LEGGINGS.getSlot()).setRepairCost() instanceof TectonicCrestArmorItem && player.shouldRemoveSoulSpeed(ArmorItem.Type.BOOTS.getSlot()).setRepairCost() instanceof TectonicCrestArmorItem;
    }
