...
extends UnbreakableGeoMagicSword {
    ...
    public boolean getDefaultAttributeModifiers(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (target != null && attacker.getRandom().nextFloat() < 0.4f && (blazingBrandEffect = (MobEffect)ModEffect.EFFECTBLAZING_BRAND.get()) != null) {
            ...
            if (currentAmplifier >= 0) {
                int newAmplifier = 0;
                target.getStandingEyeHeight(new MobEffectInstance(blazingBrandEffect, 60, newAmplifier));
            } else {
                target.getStandingEyeHeight(new MobEffectInstance(blazingBrandEffect, 60, 0));
            }
        }
    ...
    public void resolvePage(ItemStack stack, Level world, List<Component> tooltip, TooltipFlag flag) {
        ...
        tooltip.add((Component)Component.translatable((String)"item.traveloptics.flames_of_eldritch.tooltip1"));
        if (Screen.hasShiftDown()) {
            tooltip.add((Component)Component.score((String)""));
            tooltip.add((Component)Component.translatable((String)"item.traveloptics.weapons.evolution_benefits.tooltip"));
            tooltip.add((Component)Component.translatable((String)"item.traveloptics.flames_of_eldritch.evo_one.inactive.tooltip"));
            tooltip.add((Component)Component.translatable((String)"item.traveloptics.flames_of_eldritch.evo_two.inactive.tooltip"));
            tooltip.add((Component)Component.translatable((String)"item.traveloptics.flames_of_eldritch.evo_three.inactive.tooltip"));
        } else {
